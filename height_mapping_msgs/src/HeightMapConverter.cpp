...
bool HeightMapConverter::toGrayImage(const grid_map::HeightMap &map,
                                     const std::string &layer, cv::Mat &img,
  ...
  auto offset = (VALID_PIXEL_MIN * max_value - VALID_PIXEL_MAX * min_value) /
                (max_value - min_value);

  data = data * scaler +
         Eigen::MatrixXf::Constant(data.rows(), data.cols(), offset);

  // Fill NaN values with 0
  data = data.array().isNaN().select(0, data);

  // Convert to Image
  bool success = grid_map::GridMapCvConverter::toImage<unsigned char, 1>(
      map_copy, layer, CV_8UC1, 0, 255, img);
  if (!success) {
...
bool HeightMapConverter::fromGrayImage(const cv::Mat &image, float min_value,
                                       float max_value,
                                       grid_map::HeightMap &map,
                                       const std::string &layer) {
  // Set geometry of the map
  if (map.getSize().x() != image.cols || map.getSize().y() != image.rows) {
    bool success = grid_map::GridMapCvConverter::initializeFromImage(
        image, map.getResolution(), map, map.getPosition());
    if (!success)
      return false;
  }
  ...
  if (!grid_map::GridMapCvConverter::addLayerFromImage<unsigned char, 1>(
          image, layer, map, min_value, max_value))
    return false;
...
void HeightMapConverter::fromPointCloud2(const sensor_msgs::PointCloud2 &cloud,
                                         grid_map::HeightMap &map) {
  ...
  for (const auto &layer : layers) {
    if (!map.exists(layer)) {
      map.add(layer);
      std::cout << "[@ HeightMapConverter] Added " << layer
                << " layer to the height map" << std::endl;
    }
  }
  if (has_rgb) {
    if (!map.exists("r")) {
      ...
      map.add("g");
      map.add("b");
      std::cout << "[@ HeightMapConverter] Added RGB layers to the height map"
                << std::endl;
    }
  }
  ...
  std::vector<
      std::tuple<std::string, sensor_msgs::PointCloud2ConstIterator<float>>>
      iterator_fields;

  for (const auto &layer : layers)
    iterator_fields.emplace_back(
        layer, sensor_msgs::PointCloud2ConstIterator<float>(cloud, layer));

  // Populate the grid map
  for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
    grid_map::Position position(
        *iter_x, *iter_y); // Create position from point x, y values
  ...
  for (; iter_x2 != iter_x2.end(); ++iter_x2, ++iter_y2, ++iter_rgb) {
    grid_map::Position position(*iter_x2,
                                *iter_y2); // Create position from x, y values
...
void HeightMapConverter::toPointCloud2(
  ...
  for (auto &name : fieldNames) {
    ...
    offset =
        offset + pointField.count * 4; // 4 for sensor_msgs::PointField::FLOAT32
  ...
  std::unordered_map<std::string, sensor_msgs::PointCloud2Iterator<float>>
      pointFieldIterators;
  for (auto &name : fieldNames) {
    pointFieldIterators.insert(std::make_pair(
        name, sensor_msgs::PointCloud2Iterator<float>(cloud, name)));
  }
