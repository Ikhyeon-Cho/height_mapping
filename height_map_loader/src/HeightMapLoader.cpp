...
HeightMapLoader::HeightMapLoader()
{
  ...
  {
    ROS_ERROR("Could not find any .png files in the directory. Check the image path.");
    ros::shutdown();
    return;
  }
  ...
  for (const auto& img_path : image_paths)
  {
    ...
    std::filesystem::path yaml_path = img_path;
    yaml_path.replace_extension(".yaml");
    std::string layer_name = yaml_path.stem().string();  // Assumes layer name is the stem of the file name
    ...
    if (!HeightMapConverter::fromGrayImage(image, min_val, max_val, map_, layer))
    {
