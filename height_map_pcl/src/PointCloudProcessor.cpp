...
PointCloudProcessor::gridDownsampling(const pcl::PointCloud<pcl::PointXYZ>& cloud, const grid_map::HeightMap& input_map)
{
  ...
  {
    std::cout << "[PointCloudProcessor]: Frame ID mismatch - pointcloud is in a different frame! \n";
    return { false, nullptr };
  }
...
std::pair<bool, pcl::PointCloud<pcl::PointXYZ>::Ptr> PointCloudProcessor::removeInconsistentPoints(
    const pcl::PointCloud<pcl::PointXYZ>& cloud_input, const grid_map::HeightMap& map)
{
  ...
  auto cloud_consistent = boost::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
