...
{
std::pair<bool, pcl::PointCloud<pcl::PointXYZRGB>::Ptr> RGBCloudProcessor::gridDownsampling(
    const pcl::PointCloud<pcl::PointXYZRGB>& cloud, const grid_map::HeightMap& input_map)
{
  ...
  {
    std::cout << "[RGBCloudProcessor]: Frame ID mismatch - pointcloud is in a different frame! \n";
    return { false, nullptr };
  }
  ...
  auto& r_matrix = map_["r"];
  auto& g_matrix = map_["g"];
  auto& b_matrix = map_["b"];
  ...
  for (const auto& point : cloud)
  {
    ...
    r_matrix(cell_index(0), cell_index(1)) = point.r;
    g_matrix(cell_index(0), cell_index(1)) = point.g;
    b_matrix(cell_index(0), cell_index(1)) = point.b;

  }  // pointcloud loop ends

  // push back the downsampled points that were saved in the grid
  auto cloud_downsampled = boost::make_shared<pcl::PointCloud<pcl::PointXYZRGB>>();
  ...
  for (const auto& cell_index : measured_index_list)
  {
    ...
    point.r = r_matrix(cell_index(0), cell_index(1));
    point.g = g_matrix(cell_index(0), cell_index(1));
    point.b = b_matrix(cell_index(0), cell_index(1));
...
std::pair<bool, pcl::PointCloud<pcl::PointXYZRGB>::Ptr> RGBCloudProcessor::removeInconsistentPoints(
    const pcl::PointCloud<pcl::PointXYZRGB>& cloud_input, const grid_map::HeightMap& map)
{
  ...
  auto cloud_consistent = boost::make_shared<pcl::PointCloud<pcl::PointXYZRGB>>();
