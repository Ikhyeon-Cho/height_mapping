...
void HeightMap::smoothing() {
  for (grid_map::GridMapIterator iterator(*this); !iterator.isPastEnd();
       ++iterator) {
    ...
    for (grid_map::CircleIterator subiter(*this, thisGridPosition, 0.3);
         !subiter.isPastEnd(); ++subiter) {
...
const std::string &HeightMap::getHeightLayer() const { return layer_height_; }
...
bool HeightMap::isEmptyAt(const std::string &layer,
...
//         float distance_to_point = std::sqrt(ray_diff_x * ray_diff_x +
//         ray_diff_y * ray_diff_y);
...
//         for (LineIterator rayiter(*this, grid_at_robot, grid);
//         !rayiter.isPastEnd(); ++rayiter)
//         {
//             Position cell_position;
//             getPosition(*rayiter, cell_position);
//             const float cell_diff_x = cell_position.x() -
//             robotPosition2D.x(); const float cell_diff_y = cell_position.y()
//             - robotPosition2D.y(); const float distance_to_cell =
//             distance_to_point - std::sqrt(cell_diff_x * cell_diff_x +
//             cell_diff_y * cell_diff_y); const float max_height = groundHeight
//             + (robotPosition3.z() - groundHeight) / distance_to_point *
//             distance_to_cell; auto &cell_max_height = at("max_height", grid);
