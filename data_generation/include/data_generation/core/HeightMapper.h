...
public:
  ...
  HeightMapper(const Config &cfg, const HeightMap &map);

  /**
   * @brief Gridded elevation mapping using height estimator
   ...
   * @param map: heightmap
   */
  void setHeightMap(const HeightMap &map) { map_ = map; }
...
private:
  void initMap(const Config &cfg);
  void initHeightEstimator(const Config &cfg);
