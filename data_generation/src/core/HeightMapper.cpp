...
HeightMapper::HeightMapper(const Config &cfg)
    : heightFilter_{cfg.min_height, cfg.max_height} {

  initMap(cfg);
  initHeightEstimator(cfg);
}

HeightMapper::HeightMapper(const Config &cfg, const HeightMap &map)
    : map_(map), heightFilter_{cfg.min_height, cfg.max_height} {

  initHeightEstimator(cfg);
}

void HeightMapper::initMap(const Config &cfg) {
...
void HeightMapper::initHeightEstimator(const Config &cfg) {
...
typename boost::shared_ptr<pcl::PointCloud<PointT>> HeightMapper::heightMapping(
  ...
  const auto &grid_resolution = map_.getResolution();
  auto cloud_rasterized = cloudRasterization<PointT>(cloud, grid_resolution);
